...
Identify the item name, category, price, dimensions and any visible special properties.
Check the existing JSON for duplicates or variants.
Assign the next unused ID in that category.
Crop and save the item PNG.
Add the item to the correct JSON.
Verify that the JSON image path exists.
Verify that the image filename and item ID match.
